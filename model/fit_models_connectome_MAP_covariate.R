...
fit_models_connectome = function(input_file, output_file, stan_file, priors_file, fitFC, NCLUSTERS, covariate_file){
...
covariate = read.table(covariate_file, header=TRUE)
...
X = merge(X, covariate, by="Subject")
covars = colnames(covariate)[-1]
covariate = unlist(X[covars])
...
fit_parallel = function (y, x, covariate, inits, priors, mymodel, parnames){
